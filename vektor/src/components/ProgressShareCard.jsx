  ...
  const Card = () => (
    <div ref={cardRef} style={{
      ...
      <div style={{ display:'flex', justifyContent:'center', marginBottom: 60 }}>
        <div style={{ background:'#fff', borderRadius:24, padding:'20px 40px' }}>
          <img src="/logo-rd.png" alt="Rendimiento Deportivo" style={{ height:160, width:'auto', display:'block' }} crossOrigin="anonymous" />
        </div>
      </div>

      {/* TITLE */}
      <div style={{ textAlign:'center', marginBottom: 56 }}>
        <div style={{ fontSize:24, fontWeight:800, color:'#4ade80', textTransform:'uppercase', letterSpacing:'.16em', marginBottom:20 }}>
          Transformación · {months} {months === 1 ? 'mes' : 'meses'}
        </div>
        <div style={{ fontSize:108, fontWeight:900, color:'#fff', letterSpacing:'-.04em', lineHeight:.92, marginBottom:22 }}>
          {(athleteName || 'Progreso').split(' ').map((w,i) => <div key={i}>{w}</div>)}
        </div>
        <div style={{ fontSize:26, color:'#555', marginTop:12 }}>{fmtDate(first.date)} → {fmtDate(last.date)}</div>
      </div>

      {/* PESO HERO */}
      <div style={{ background:'rgba(74,222,128,0.07)', border:'1px solid rgba(74,222,128,0.2)', borderRadius:32, padding:'50px 60px', marginBottom:36, display:'flex', justifyContent:'space-between', alignItems:'center' }}>
        <div>
          <div style={{ fontSize:20, fontWeight:800, color:'#4ade80', textTransform:'uppercase', letterSpacing:'.1em', marginBottom:16 }}>Peso corporal</div>
          <div style={{ display:'flex', alignItems:'baseline', gap:24 }}>
            <div>
              <div style={{ fontSize:18, color:'#444', marginBottom:6 }}>Inicio</div>
              <div style={{ fontSize:54, fontWeight:700, color:'#555', fontFamily:'monospace' }}>{parseFloat(first.weight).toFixed(1)}<span style={{ fontSize:20, color:'#444', marginLeft:5 }}>kg</span></div>
            </div>
            <div style={{ fontSize:36, color:'#2a2a2a', marginBottom:4 }}>→</div>
            <div>
              <div style={{ fontSize:18, color:'#888', marginBottom:6 }}>Hoy</div>
              <div style={{ fontSize:84, fontWeight:900, color:'#fff', fontFamily:'monospace', lineHeight:1 }}>{parseFloat(last.weight).toFixed(1)}<span style={{ fontSize:24, color:'#666', marginLeft:5 }}>kg</span></div>
            </div>
          </div>
          {d('weight') != null && <div style={{ marginTop:18 }}><DeltaBadge value={d('weight')} unit="kg" invert={false} /></div>}
        </div>
        <SparkLine data={weightData} color="#4ade80" width={300} height={140} />
      </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:28, flex:1 }}>
        {[
          ...
          <div key={key} style={{ background:'rgba(255,255,255,0.03)', border:'1px solid rgba(255,255,255,0.07)', borderRadius:26, padding:'36px 32px 28px', display:'flex', flexDirection:'column', justifyContent:'space-between' }}>
            <div style={{ fontSize:17, fontWeight:800, color:'#555', textTransform:'uppercase', letterSpacing:'.08em', marginBottom:16 }}>{label}</div>
            <SparkLine data={data} color={color} width={390} height={85} />
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-end', marginTop:16 }}>
              <div>
                <div style={{ fontSize:16, color:'#333', marginBottom:5 }}>Inicio</div>
                <div style={{ fontSize:34, fontWeight:700, color:'#555', fontFamily:'monospace' }}>{parseFloat(first[key])?.toFixed(1) || '—'}</div>
              </div>
              <div style={{ textAlign:'right' }}>
                <div style={{ fontSize:16, color:'#555', marginBottom:5 }}>Hoy</div>
                <div style={{ fontSize:52, fontWeight:900, color:'#fff', fontFamily:'monospace', lineHeight:1 }}>{parseFloat(last[key])?.toFixed(1) || '—'}<span style={{ fontSize:18, color:'#555', marginLeft:3 }}>{unit}</span></div>
              </div>
            </div>
            {d(key) != null && <div style={{ marginTop:14 }}><DeltaBadge value={d(key)} unit={unit} invert={invert} /></div>}
          </div>
        ))}
      </div>

      {/* FOOTER */}
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', paddingTop:36, marginTop:36, borderTop:'1px solid rgba(255,255,255,0.06)' }}>
        <div style={{ fontSize:19, color:'#2a2a2a', fontWeight:600 }}>Seguimiento profesional de composición corporal</div>
        <div style={{ background:'#fff', borderRadius:14, padding:'8px 18px' }}>
          <img src="/logo-rd.png" alt="Rendimiento Deportivo" style={{ height:60, width:'auto', display:'block' }} crossOrigin="anonymous" />
        </div>
      </div>
    </div>
  )
